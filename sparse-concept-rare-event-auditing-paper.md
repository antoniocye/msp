...
The main difference is that ARC's strongest aspiration is mechanistic estimation of an expectation from structural understanding. This paper's \(G\) still samples labels. The experiment tests whether mechanistic-looking structure can make sampling more efficient, not whether it can replace sampling.

## 3. The Explanation \(\pi\)

The explanation is a sparse activation dictionary and associated atom predictors reused across queries. It is built once, before the individual audit queries are evaluated.
...
Operationally, the build procedure is separated from per-query estimation:
...
At 512 labels, the sparse-internal estimator has `56.1%` of Monte Carlo RMSE, or `31.5%` of Monte Carlo MSE. This is the strongest same-budget result reported here. At 2048 labels, Monte Carlo has less variance, and the sparse-internal advantage is no longer clear.
...
The estimator that wins depends on the query family. This heterogeneity is expected because the query families use different information sources.
...
![CIFAR-10 archived run: query-family RMSE ratios](experiments/results/stronger/cifar10_sparse_concept_family_breakdown.png)

The result is not "sparse bases always win." The result is better stated as context-dependent. Sparse internal atoms help most when the query refers to sparse internal structure. Output scores help when the failure is already visible in confidence or predicted class. Hand-defined image concepts help when the query is written in those concepts.
...
![CIFAR-10 archived run: activation intervention effects](experiments/results/stronger/cifar10_sparse_concept_interventions.png)

This supports a limited interpretation: the sparse basis captures directions that are predictive and interventionally associated with some classifier behavior. It does not show that each component is a crisp human concept or that the dictionary is sufficient for arbitrary audits.
...
- The experiment implements an explicit build procedure \(E\) that produces reusable sparse audit advice \(\pi\) from model activations.
...
The central claim is limited to the measured workload: reusable structural advice can make rare-failure sampling more efficient here.
